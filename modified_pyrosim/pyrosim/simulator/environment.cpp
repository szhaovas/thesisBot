...
void ENVIRONMENT::Read_From_Python(dWorldID world, dSpaceID space, Data *data)
{
       char incomingString[10000];
        ...
        while ( strcmp(incomingString,"Done") != 0 ) {
				...
				else if ( strcmp(incomingString,"CTRNN") == 0){
						Create_CTRNN();
				}
                //Synapse
                else
...
void ENVIRONMENT::Create_CTRNN(void)
{
	size_t ctrnnSize; std::cin >> ctrnnSize;
	size_t ctrnnWtsSize; std::cin >> ctrnnWtsSize;
	size_t ctrnnInSize; std::cin >> ctrnnInSize;
	size_t ctrnnOutSize; std::cin >> ctrnnOutSize;

	std::vector<std::vector<int>> connections;
	for (size_t i = 0; i < ctrnnSize; i++) {
		std::vector<int> temp;
		for (size_t j = 0; j < ctrnnSize; j++) {
			int conn; std::cin >> conn;
			temp.push_back(conn);
		}
	...
	for (size_t i = 0; i < ctrnnWtsSize; i++) {
	...
	std::vector<int> ctrnnInID;
	for (size_t i = 0; i < ctrnnInSize; i++) {
		int id; std::cin >> id;
		ctrnnInID.push_back(id);
	}

	std::vector<int> ctrnnOutID;
	for (size_t i = 0; i < ctrnnOutSize; i++) {
		int id; std::cin >> id;
		ctrnnOutID.push_back(id);
	}

	double step_size; std::cin >> step_size;

	if ( neuralNetwork == NULL )
		Create_Neural_Network();

	neuralNetwork->Add_CTRNN(connections, wts, taus, biases, gains, step_size, ctrnnInID, ctrnnOutID);
}
...
void ENVIRONMENT::Create_Proprioceptive_Sensor(int evalPeriod) {
	...
	char locheck[10]; std::cin >> locheck;
	double lo; std::cin >> lo;

	if (strcmp(locheck,"True")==0)
	{
		joints[jointIndex]->Create_Proprioceptive_Sensor(ID,evalPeriod,true,lo);
	} else
	{
		joints[jointIndex]->Create_Proprioceptive_Sensor(ID,evalPeriod,false,lo);
	}
}
