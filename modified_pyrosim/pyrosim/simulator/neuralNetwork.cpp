...
void NEURAL_NETWORK::Add_CTRNN(std::vector<std::vector<int>> connections,
...
{
	CTRNN *ctrnn = new CTRNN(connections, wts, taus, biases, gains, step_size);
	this->ctrnn = ctrnn;
	this->ctrnnInID = ctrnnInID;
	this->ctrnnOutID = ctrnnOutID;
}
...
void NEURAL_NETWORK::Update(int timeStep) {
		...
		if (ctrnn) {
			std::vector<double> ctrnnInput;
			for (size_t i = 0; i < ctrnnInID.size(); i++)
			{
				ctrnnInput.push_back(neurons[ctrnnInID[i]]->Get_Value());
			}
			ctrnn->step(ctrnnInput);

			std::vector<double> ctrnnOutput = ctrnn->get_outputs();
			for (size_t i = 0; i < ctrnnOutID.size(); i++)
			{
				// sigmoid ranges between 0 and 1
				// we need range between +-1
				neurons[ctrnnOutID[i]]->Set(ctrnnOutput[ctrnnInput.size()+i]*2-1);
			}
		} else {
			Update_Neurons();
			Threshold_Neurons();
