...
# ----------Sensors----------------------
    ...
    def send_proprioceptive_sensor(self, joint_id=0, locheck=False, lo=0):
...
# ------CTRNN------------------------------
    def send_ctrnn(self, connections, wts, taus,
                   input_neuron_ids, output_neuron_ids,
                   biases=None,
                   gains=None,
                   step_size=None):

        assert self._ctrnn_sent is False
        assert connections.shape[0] == connections.shape[1]
        size = connections.shape[0]
        assert len(taus) == size
        assert len(input_neuron_ids)+len(output_neuron_ids) <= size
        ...
        send_string = []
        # connections
        iterator = np.nditer(connections)
        for element in iterator:
            send_string.append(int(element))
        # wts
        send_string.extend(wts)
        # taus, biases, gains
        send_string.extend(taus)
        send_string.extend(biases)
        send_string.extend(gains)
        # input, output
        send_string.extend(input_neuron_ids)
        send_string.extend(output_neuron_ids)
        # step size
        send_string.append(step_size)

        self._send('CTRNN', size, len(wts), len(input_neuron_ids),
                   len(output_neuron_ids), *send_string)

        self._ctrnn_sent = True
